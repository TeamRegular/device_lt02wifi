...
static int coda_info_evaluate(const void *s) {
	...
	if (strcmp(keyword, "high_framerate") == 0) {
		int high_framerate = 0;
        evaluate_value = atoi(value);
		if((cvf->strm_fmt == STRM_FMT_MPG4)||(cvf->strm_fmt == STRM_FMT_VC1)
            ||(cvf->strm_fmt == STRM_FMT_VC1M))
			high_framerate = 1;

		return (evaluate_value == high_framerate) ? MATCH : DONT_MATCH;
	}
