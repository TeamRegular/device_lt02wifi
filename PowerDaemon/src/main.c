	...
	while (1) {
		...
		if(FD_ISSET(read_fd, &readfds)) {
            ...
            if (cnt == sizeof(head)) {
                plugin_tag = head[0];
                data_len = head[1];
                clog(LOG_DEBUG, "read socket %d, [tag]=%d, [len]=%d.", read_fd, plugin_tag, data_len);
...
/*
 * check whether the keywords in the rule is updated
 */
...
static void ppd_loop(PowerPolicyDaemonConf *config) {
    ...
    /* constraint voting */
	clog(LOG_DEBUG, "<========= start constraint voting =========>");
	pthread_mutex_lock(&config->ppd_mutex);
...
int main (int argc, char *argv[]) {
	...
	fd_set rfds;
	int ppd_sock = -1, peer_sock = -1; /* input pipe */
